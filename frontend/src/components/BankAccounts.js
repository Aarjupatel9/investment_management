...
const BankAccounts = () => {
  ...
  useEffect(() => {
    const eventPromise = bankService.getBankDetails();
    toast.promise(
      eventPromise,
      {
        loading: "fetching Bank details",
        success: "",
        error: (err) => err,
      },
      {
    ...
    eventPromise
      .then((res) => {
        console.log("users : ", res.bank);
        setBankDetails(res.bank);
      })
      .catch((error) => {
  ...
  const handelEventDelete = (_id) => {
    const eventPromise = bankService.deleteBankDertail(_id);
    eventPromise
      .then((res) => {
        console.log("users : ", res);
        const tmp = bankDetails.filter((qualification) => {
      ...
      {bankDetails.length > 0 ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right dark:text-white bg-gray-100 dark:bg-gray-800">
                        <div
                          onClick={() => {
                            navigate("/editBankAccount/" + qualification._id);
                          }}
        ...
        <div className="flex flex-col pt-4">
          <h1 className="mx-auto  text-2xl font-bold text-gray-900 dark:text-white">
            There is no BankAccount Details
          </h1>
        </div>
      )}
    </div>
