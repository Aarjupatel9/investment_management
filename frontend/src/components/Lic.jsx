...
const Lic = () => {
  ...
  useEffect(() => {
    const fdPromise = licService.getUserLicDetails();
    toast.promise(
      fdPromise,
      {
        loading: "fetching Lic details",
        success: "",
        error: (err) => err,
      },
      {
  ...
  const handelEventDelete = (_id) => {
    const eventPromise = licService.deleteLicDetail(_id);
    eventPromise
      .then((res) => {
        console.log("users : ", res);
        const tmp = licDetails.filter((qualification) => {
    ...
    toast.promise(
      eventPromise,
      {
        loading: "please wait while we deleting Lic Details",
                ...
                {licDetails.map((qualification) => {
                  return (
                    ...
                    >
                      ...
                      >
                        {qualification.policyNumber}
                      </th>
                      <td className="px-6 py-4 dark:text-white dark:bg-gray-700">
                        {qualification.insuranceAmmount}
                      </td>
                      <td className="px-6 py-4 bg-gray-100 dark:bg-gray-800">
                        {formatDateToDdMmYyyy(String(qualification.maturityDate))}
                      </td>
                      <td className="px-6 py-4 dark:text-white dark:bg-gray-700">
                        {qualification.sumAssured}
                      </td>
                      <td className="px-6 py-4 bg-gray-100 dark:bg-gray-800">
                        {formatDateToDdMmYyyy(String(qualification.premiumDate))}
                      </td>
                      <td className="px-6 py-4 text-right dark:text-white dark:bg-gray-700">
                        <div
                          onClick={() => {
                            navigate("/editLic/" + qualification._id);
                          }}
